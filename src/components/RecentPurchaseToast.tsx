...
export const RecentPurchaseToast: React.FC = () => {
  ...
  return (
    <div className="hidden sm:flex fixed bottom-5 left-5 z-40 max-w-sm rounded-2xl bg-[#11131a]/95 border border-purple-500/30 p-3.5 shadow-2xl backdrop-blur-md items-center gap-3 animate-slideIn">
      <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-purple-600 via-pink-600 to-amber-500 flex items-center justify-center text-white shrink-0 shadow-lg shadow-purple-600/30">
        <ShoppingBag className="w-5 h-5" />
      </div>

      <div className="min-w-0 flex-grow pr-2">
        <div className="flex items-center gap-1.5 text-xs text-white font-bold">
          <span className="text-purple-300">{current.name}</span>
        ...
        <p className="text-[11px] text-emerald-400 font-medium truncate flex items-center gap-1">
          <CheckCircle2 className="w-3 h-3" /> Purchased {current.pack}
        </p>
        <span className="text-[10px] text-gray-500">{current.time} • Verified Buyer</span>
      </div>
