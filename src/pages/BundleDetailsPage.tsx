...
export const BundleDetailsPage: React.FC = () => {
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-24 text-center min-h-screen">
        <div className="p-8 sm:p-10 rounded-3xl bg-white border border-slate-200 shadow-xl shadow-slate-200/50">
          <Film className="w-12 h-12 text-purple-600 mx-auto mb-4" />
          <h2 className="text-2xl font-black text-slate-900 mb-2">Bundle Unavailable</h2>
          <p className="text-slate-500 text-sm mb-6">{error || 'This video bundle is no longer listed.'}</p>
          <Link
            to="/bundles"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-sm font-bold shadow-lg shadow-purple-600/20 transition-all"
          >
  ...
  return (
    <div className="py-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-screen text-slate-900">
      {showCheckout && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-7 space-y-8">
          {/* Main Visual */}
          <div className="relative aspect-video rounded-3xl overflow-hidden bg-slate-900 border border-slate-200 shadow-2xl shadow-slate-300/40">
            <img
            ...
            <div className="absolute top-4 left-4 flex items-center gap-2">
              <span className="px-3 py-1 rounded-full bg-purple-600/90 text-white font-black text-xs shadow">
                {bundle.category}
              </span>
              {discountPercent > 0 && (
                <span className="px-3 py-1 rounded-full bg-pink-600 text-white font-black text-xs shadow">
                  {discountPercent}% OFF
                </span>
              )}
            </div>
            ...
            <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between text-xs text-white">
              ...
              <span className="bg-black/60 backdrop-blur-md px-3 py-1 rounded-lg border border-white/20 font-bold">
                {bundle.resolution}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-sm">
              <MonitorPlay className="w-5 h-5 text-purple-600 mb-2" />
              <div className="text-[11px] text-slate-400 uppercase font-black tracking-wider">Resolution</div>
              <div className="text-sm font-black text-slate-900 mt-0.5">{bundle.resolution}</div>
            </div>

            <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-sm">
              <FileVideo className="w-5 h-5 text-indigo-600 mb-2" />
              <div className="text-[11px] text-slate-400 uppercase font-black tracking-wider">Format</div>
              <div className="text-sm font-black text-slate-900 mt-0.5">MP4 / ProRes 60FPS</div>
            </div>

            <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-sm">
              <HardDrive className="w-5 h-5 text-pink-600 mb-2" />
              <div className="text-[11px] text-slate-400 uppercase font-black tracking-wider">File Size</div>
              <div className="text-sm font-black text-slate-900 mt-0.5">{bundle.fileSize}</div>
            </div>

            <div className="p-4 rounded-2xl bg-white border border-slate-200/80 shadow-sm">
              <Clock className="w-5 h-5 text-amber-500 mb-2" />
              <div className="text-[11px] text-slate-400 uppercase font-black tracking-wider">Delivery</div>
              <div className="text-sm font-black text-slate-900 mt-0.5">Instant 10-Min Session</div>
            </div>
          </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="p-7 sm:p-8 rounded-3xl bg-white border-2 border-purple-500/80 sticky top-24 shadow-2xl shadow-purple-500/10">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-100 border border-purple-200 text-purple-700 text-xs font-bold uppercase tracking-wider mb-4">
            ...
            {/* Price section */}
            <div className="p-4 rounded-2xl bg-purple-50/60 border border-purple-100 mb-6">
              <div className="flex items-baseline justify-between mb-1">
                <div className="flex items-baseline gap-3">
                  <span className="text-3xl sm:text-4xl font-black text-slate-900">₹{bundle.price}</span>
                  {bundle.originalPrice > bundle.price && (
                    <span className="text-sm text-slate-400 line-through">₹{bundle.originalPrice}</span>
                  )}
                </div>
                {discountPercent > 0 && (
                  <span className="text-xs font-black text-pink-700 bg-pink-100 px-2.5 py-1 rounded-full border border-pink-200">
                    Save {discountPercent}%
                  </span>
                )}
              </div>
              <p className="text-[11px] text-slate-500">One-time payment • No monthly subscription</p>
            </div>

            {/* Instant Checkout CTA */}
            <button
              onClick={() => setShowCheckout(true)}
              className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-purple-600 via-pink-600 to-amber-500 hover:from-purple-500 hover:to-amber-400 text-white font-black text-base shadow-xl shadow-purple-600/30 flex items-center justify-center gap-2.5 transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
            ...
            {/* Guarantees */}
            <div className="mt-6 space-y-2.5 text-xs text-slate-600 border-t border-slate-100 pt-6">
